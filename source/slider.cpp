...
{
    if ( (m_bSelStart || m_bSelEnd) && m_bMousePress )
        return;
...
void CSlider::paintEvent(QPaintEvent *)
{
    ...
    if (m_bTimeMode)
    {
        QRect rcStartTime(rcClient);
        rcStartTime.setTop(m_rcBg.top() - 6);
        rcStartTime.setRight(m_rcBg.left() - 10);
        painter.setPen(QColor("#C6C6C6"));

        int nMin = 0;
        int nSec = 0;
        if (itemEnd.m_nValue > 0)
        {
            nMin = itemEnd.m_nValue / 60;
            nSec = itemEnd.m_nValue % 60;
        }

        QString strCurTime = QString("%1:%2").arg(nMin, 2, 10, QLatin1Char('0')).arg(nSec, 2, 10, QLatin1Char('0'));
        painter.drawText(rcStartTime, Qt::AlignRight | Qt::AlignTop, strCurTime);

        QRect rcEndTime(rcStartTime);
        rcEndTime.setLeft(m_rcBg.right() + 10);
        rcEndTime.setRight(rcClient.right());
        ...
        QString strEndTime = QString("%1:%2").arg(nMin, 2, 10, QLatin1Char('0')).arg(nSec, 2, 10, QLatin1Char('0'));
        painter.drawText(rcEndTime, Qt::AlignLeft | Qt::AlignTop, strEndTime);
    }
}
...
void CSlider::UpdateItemList()
{
    ...
    if (m_bTimeMode)
    {
        m_rcBg.setLeft(m_rcBg.left() + 50);
        m_rcBg.setRight(m_rcBg.right() - 50);
    }
